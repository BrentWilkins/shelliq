...
fn real_curated_conversion_is_deterministic_and_lossless() {
    ...
    assert_eq!(first.manifest.total_records, 786);
    assert_eq!(first.manifest.converted_records, 786);
    assert!(first.manifest.cst_exempt_record_ids.is_empty());
    assert!(first.manifest.semantic_exempt_record_ids.is_empty());
    assert_eq!(first.manifest.normalized_render_record_ids.len(), 21);
    assert_eq!(first.manifest.input_files.len(), 17);
    assert!(
        first
            .manifest
