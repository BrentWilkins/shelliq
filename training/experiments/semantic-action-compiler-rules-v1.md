...
3. BSD `xargs` empty-input safety: on Darwin, when the instruction requests safe handling of an empty input list and context explicitly says GNU `-r` is unsupported, emit `xargs -0 rm --`.
...
- Unit tests cover positive and negative rule matching, documented option order, protocol exclusion, and Rust action encoding.
