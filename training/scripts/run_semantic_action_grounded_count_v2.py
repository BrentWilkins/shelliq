#!/usr/bin/env python3
"""Run the preregistered soft grounded retrieval/count hybrid."""
...
EXPERIMENT = 'semantic-action-grounded-count-v2'
ROLE_COUNT = 8


def build_model(tokenizer, *, dropout: float = 0.1) -> SemanticActionSoftGroundedCountModel:
    pretrained = AutoModelForSeq2SeqLM.from_pretrained(
        typed.candidate.MODEL_ID,
        revision=typed.candidate.REVISION,
        local_files_only=True,
    )
    return SemanticActionSoftGroundedCountModel.from_codet5(
        pretrained,
        tokenizer,
