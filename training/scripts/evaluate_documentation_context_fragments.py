#!/usr/bin/env python3
"""Evaluate prompt-context option fragments over typed documentation bases."""
...
EXPERIMENT = 'semantic-action-context-fragments-v1'
...
def main() -> None:
    ...
    for record in evaluation:
        ...
        documents.extend(candidate.document for candidate in candidates)
        rows.append(
            {
                'record_id': record.record_id,
                'command': record.command,
                'retrieved_templates': len(retrieved),
                'context_options': list(documented_context_options(record.context)[:6]),
    ...
    decoded = client.decode(client.encode(documents))
    ...
    gate_passed = (
        valid == len(documents) and metrics['skeleton_oracle'] >= 19 and metrics['skeleton_oracle_distinct_commands'] >= 10
    )
    report = {
        'schema_version': 1,
        'experiment': EXPERIMENT,
        'phase': 'inner-context-composition-feasibility',
        'gate_passed': gate_passed,
        'retrieval_limit': 8,
        'composition_limit': 512,
        'maximum_context_options': 6,
        'metrics': metrics,
        'floors': {
    ...
    if not gate_passed:
        raise SystemExit('authoritative-context fragment gate failed')
